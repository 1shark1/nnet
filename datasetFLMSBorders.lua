...
function Dataset(fname, settings)
  ...
  dataset.index.file = torch.Tensor(totalSamples):int();
  dataset.index.pos = torch.Tensor(totalSamples):int();
  ...
  for ll = 1, #dataset.nSamplesList, 1 do   
    ...
    if (nSamples >= 1) then
      i = settings.seqL;
      dataset.index.file:narrow(1, dataset.nSamples + 1, nSamples):fill(ll);
      dataset.index.pos:narrow(1, dataset.nSamples + 1, nSamples):apply(function(x)
        i = i + 1;
        return i;
      end);
