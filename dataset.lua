...
function Dataset(list, isFilelist, computeFramestats, loadPackageData, savePackageData, decode)
  ...
  if not loadPackageData then
    ...
    for file = 1, #filelist, 1 do  
      ...
      globalSamples = globalSamples + nSamples
      nSamples = nSamples - settings.seqL - settings.seqR
    ...
    if savePackageData then
      ...
      savePackage(dataset.cache, dataset.nSamplesList, globalSamples, totalSamples, #filelist, listName)      
  ...
  -- load saved package
    ...
    local nSamples, sampPeriod, sampSize, parmKind, fvec, references, filesCount
    nSamples, sampPeriod, sampSize, parmKind, fvec, references, globalSamples, totalSamples, filesCount, dataset.nSamplesList = readPackage(listName)    
    fvec = fvec:view(fvec:size(1) * settings.inputSize)
    table.insert(dataset.cache, { inp = fvec, out = references })
  end
  ...
  if not loadPackageData then 
    ...
    for ll = 1, #dataset.nSamplesList, 1 do   
      ...
      if nSamples >= 1 then     -- sanity check
        i = i + settings.seqL
        dataset.index.file:narrow(1, dataset.nSamples + 1, nSamples):fill(1)                
        dataset.index.pos:narrow(1, dataset.nSamples + 1, nSamples):apply(function(x)       -- fe [1 2 3 13 14 15] + seqL
          i = i + 1
          return i
        end)
        i = i + settings.seqR
